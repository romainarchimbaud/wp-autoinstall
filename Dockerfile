...
# Install make
RUN apt update && \
  apt install -y make less bash-completion default-mysql-client
RUN echo "[ -f /etc/bash_completion ] && . /etc/bash_completion" >> /var/www/.bashrc

# Install nodejs / npm : seulement si INSTALL_NODE=true dans .env
ARG INSTALL_NODE=false
RUN if [ "$INSTALL_NODE" = "true" ]; then apt-get install -y nodejs npm; fi && \
